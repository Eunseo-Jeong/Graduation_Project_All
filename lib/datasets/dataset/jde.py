...
class MultiLoadVideo:  # for inference
    def __init__(self, lv1, lv2, img_size=(1088, 608)):
        self.lv1 = lv1
        self.lv2 = lv2
        self.full_len = len(self.lv1) + len(self.lv2)
    ...
    def __next__(self):
        ...
        if self.count % 2 == 0:
            data = self.lv1[int(self.count/2)]
        else:
            data = self.lv2[int(self.count/2)]

        # cv2.imwrite(img_path + '.letterbox.jpg', 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        return data

    def __len__(self):
        return self.full_len  # number of files
